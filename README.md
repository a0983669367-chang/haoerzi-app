...
| 變數 | 顏色 | 用途 | 對比度 |
|---|---|---|---|
| `--head` | `#0F4C75` | **標題、金額、重點數字**（深藍） | 白底 9.09:1 |
| `--navy` | `#DB190E` | 品牌紅：logo、選中的頁籤／導覽、主要按鈕、金額卡色條 | 白字 5.05:1 |
| `--gold` | `#E9A227` | 亮金，只用來「填色」：金色按鈕底、月曆金點 | 配深字 6.79:1 |
| `--gold-tx` | `#8A5510` | 深琥珀，只用來「當文字」放在白底上 | 白底 6.2:1 |
| `--teal` | `#1B7B8C` | 青色：滑桿、聯絡類按鈕（白字）、營業員視角 | 白字 4.93:1 |
| `--red` | `#7A2233` | 警示色：詐騙警告、165 按鈕、RR5 | 白字 9.97:1 |
| `--bg` | `#FFFFFF` | 桌機頁面底色：全白 | — |
| `--tint-2` | `#E8EBEF` | 手機標題列的灰底 | — |
| `--tint` | `#F2F4F6` | 手機螢幕底色、卡片內填充 | — |

**紅色不拿來當內文顏色。** 標題、金額、重點數字一律用深藍 `--head`；
紅色只保留給品牌識別與行動——logo、選中的頁籤／導覽、主要按鈕、金額卡左側色條。
滿頁紅字對長輩來說太刺眼，也會跟真正的警告混淆。

按鈕依用途分色：紅＝真正會成交的動作（確認送出、預約營業員）、
金＝探索與試算（選方案、試算並申購）、青＝聯絡人（撥電話、預約回電，白字）。
