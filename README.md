...
| 變數 | 顏色 | 用途 |
|---|---|---|
| `--navy` | `#0F4C75` | 主色：主要按鈕、選中的頁籤／導覽、卡片色條 |
| `--head` | `#0F4C75` | 標題、金額、重點數字（同一個藍，語意不同所以分開放） |
| `--bg` | `#E9ECEF` | 頁面底色：灰 |
| `--tint-2` | `#E4E8EC` | 手機標題列的灰 |
| `--tint` | `#F3F5F7` | 手機螢幕底色、卡片內填充 |
| `--card` | `#FFFFFF` | 卡片：白 |
| `--ink` / `--ink-2` | `#1F2A37` / `#5B6A7A` | 主要／次要文字 |

**整體只用灰、白、深藍三個色階**，顏色只留給有特殊意義的東西：

| 語意色 | 用在哪裡 |
|---|---|
| 金 `--gold` / `--gold-tx` | ① 月曆的入帳金點、④ 每月約領與申購試算金額；**「加入模擬方案」後整張金額卡會變成金色實底＋白字**，在整頁白卡中最搶眼 |
| 綠 `--green` | 安全、適合您（正向狀態） |
| 琥珀 `--gold-lt` | 警示、RR4、需加強評估 |
| 酒紅 `--red` | 高風險、詐騙警告、RR5 |
| 青 `--teal` | 聯絡真人（撥電話、預約回電） |
| 品牌紅 `--brand-red` | 只給吉祥物「小方」的領巾與鞋 |

介面本身不再用紅色。logo 是圖檔，保留官方原本的紅黑配色不受影響。
